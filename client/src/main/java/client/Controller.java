...
public class Controller implements Initializable {
    ...
    //Инициализация лог-файла на стороне клиента
    private FileOutputStream initLogFile() throws IOException {
        String logFileName = login+".txt";
        File logFile = new File("LogClient/"+logFileName);

        if(!logFile.exists()){
            logFile.createNewFile();
        }

        FileOutputStream fos = new FileOutputStream("LogClient/"+logFileName,true);
        return fos;
    }

    //Запись сообщений в лог-файл на стороне клиента
    private void writeLogFile(String msg, FileOutputStream fos) throws IOException {
        fos.write((msg+"\n").getBytes());
    }
